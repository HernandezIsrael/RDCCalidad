//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Model
{
    using System;
    using System.Collections.Generic;
    
    public partial class Ca_Nivel2
    {
        public int Id_Nivel2 { get; set; }
        public int Id_Nivel1 { get; set; }
        public string Nivel2 { get; set; }
        public Nullable<bool> Subcidio { get; set; }
        public Nullable<decimal> Monto_Subcidio { get; set; }
        public Nullable<decimal> Monto_Sin_Subcidio { get; set; }
        public Nullable<int> Id_Moneda { get; set; }
        public Nullable<int> Creado_por { get; set; }
        public Nullable<System.DateTime> F_Alta { get; set; }
        public Nullable<bool> Activo { get; set; }
    }
}
